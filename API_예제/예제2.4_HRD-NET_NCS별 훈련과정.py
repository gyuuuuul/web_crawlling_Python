...
# 사용자 입력 받기
ncs1 = input("NCS 직종 대분류 코드 (전체일 경우 옵션 생략) : ").strip()
...
def fetch_training_data(auth_key, request_url, category_name):
    ...
    while True:
        ...
        # ✅ 국민내일배움카드는 NCS 코드가 `srchNcs1`로 다름
        if category_name == "국민내일배움카드":
            params["srchNcs1"] = ncs1
        else:
            params["srchKeco1"] = ncs1
...
excel_filename = os.path.join(desktop_path, f"{ncs1 + '_' + start_date + '-' + end_date }_훈련과정.xlsx")
